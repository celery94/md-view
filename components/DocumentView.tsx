...
export default function DocumentView({ content, theme, onThemeChange, onClose }: DocumentViewProps) {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-sm print:hidden" aria-hidden="true" />
      ...
      <div className="relative z-10 flex flex-1 justify-center overflow-y-auto px-4 py-6 sm:px-8 sm:py-10 print:p-0 print:overflow-visible">
        <div
          ref={panelRef}
          tabIndex={-1}
          className="document-view-panel relative flex w-full max-w-4xl flex-1 flex-col overflow-hidden rounded-3xl bg-white shadow-2xl ring-1 ring-slate-200 focus:outline-none print:max-w-none print:flex-auto print:rounded-none print:shadow-none print:ring-0"
        >
          <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3 sm:px-6 print:hidden">
            <div>
              <h2 className="text-base font-semibold text-slate-900">Document View</h2>
              <p className="text-xs text-slate-500">Clean layout for printing or saving to PDF</p>
            </div>
